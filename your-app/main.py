...
@app.get("/losts")  # GET /todos にアクセスされたら実行
def get_losts():
    """TODO一覧を取得する"""
...
@app.post("/losts", status_code=201)  # POST /todos で新規作成（201=作成成功）
def create_todo(todo: lostsCreate):
    """新しいTODOを作成する"""
...
def update_todo(todo_id: int, todo: lostsUpdate):
    """TODOの完了状態を更新する"""
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    ...
    cursor.execute(
        "UPDATE losts SET packed = ? WHERE id = ?",
        (int(todo.packed), todo_id),
    )
    conn.commit()  # 更新を確定する
...
def delete_todo(todo_id: int):
    ...
    cursor.execute("SELECT id FROM losts WHERE id = ?", (todo_id,))
    existing = cursor.fetchone()
    if existing is None:
        conn.close()
        raise HTTPException(status_code=404, detail="TODO not found")

    cursor.execute("DELETE FROM losts WHERE id = ?", (todo_id,))  # 削除する
    conn.commit()  # 削除を確定する
